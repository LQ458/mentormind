...
export function HighlightAskAI({ containerRef, subject, unitTitle, language, getAuthHeaders }: AskAIProps) {
  ...
  const handleAsk = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setError(data.error || (isZh ? '获取回答失败' : 'Failed to get answer'))
      }
    } catch {
        ...
        {!showInput && !answer ? (
          ...
          <div className="p-3 space-y-2">
            ...
            {!answer ? (
              <div className="flex gap-2">
                <input
                  ...
                  placeholder={isZh ? '问问这段内容…' : 'Ask about this...'}
            ...
            {answer && (
              ...
              >
                {isZh ? '继续提问' : 'Ask another question'}
              </button>
            )}
          </div>
        )}
...
export function ScreenshotAskAI({ containerRef, subject, unitTitle, language, getAuthHeaders }: AskAIProps) {
  ...
  const handleAsk = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/backend/study-plan/ask-ai', {
        ...
        body: JSON.stringify({
          image_base64: base64,
          question: question.trim() || (isZh ? '解释这张截图里的内容。' : 'Explain what is shown in this image.'),
          subject,
          unit_title: unitTitle,
          language,
        }),
      ...
      {capturing && (
        ...
        >
          <div className="absolute top-3 left-1/2 -translate-x-1/2 bg-black/70 text-white text-xs px-3 py-1.5 rounded-full">
            {isZh ? '拖拽选择区域，按 ESC 取消' : 'Drag to select an area \u2014 ESC to cancel'}
          </div>
          {cropRect && (
            <div
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={handleClose}>
          <div className="bg-white rounded-xl shadow-2xl border border-gray-200 max-w-lg w-full mx-4 overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="p-4 border-b border-gray-100 flex items-center justify-between">
              <h3 className="text-sm font-semibold text-gray-900">{isZh ? '问 AI 这张截图' : 'Ask AI about this screenshot'}</h3>
              ...
              {!answer ? (
                <div className="flex gap-2">
                  <input
                    ...
                    onChange={(e) => setQuestion(e.target.value)}
                    onKeyDown={(e) => { if (e.key === 'Enter') handleAsk() }}
                    placeholder={isZh ? '问问这张图…（或直接点提问）' : 'Ask about this image... (or press Ask for explanation)'}
                  ...
                  >
                    {loading ? (isZh ? '思考中...' : 'Thinking...') : (isZh ? '提问' : 'Ask')}
                  </button>
                </div>
              ) : (
